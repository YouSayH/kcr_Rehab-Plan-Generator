...
def _build_group_prompt(group_schema: type[BaseModel], patient_facts_str: str, generated_plan_so_far: dict) -> str:
    ...
    return textwrap.dedent(f"""
        ...
        - **FIM整合性**: 患者データにFIMの点数が含まれている場合、上記の【FIMの点数目安】を厳密に参照し、**点数の定義（介助量や自立度）と矛盾しない文章**にしてください。（例：4点なのに「全介助」と書かない、6点なのに「手助けが必要」と書かない等
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        print(
                            f"APIレート制限またはサーバーエラー: {e}。{wait_time}秒後に再試行します... ({attempt + 1}/{max_retries})"
                        )
