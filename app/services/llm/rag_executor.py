...
class RAGExecutor:
        ...
        for name, config in specific_config.items():
            if config:
                ...
                if "llm" in params and isinstance(params["llm"], dict):
                    ...
                    params["llm"] = get_instance(llm_cfg["module"], llm_class, llm_cfg.get("params", {}))
    ...
    def _construct_prompt(self, patient_facts_dict: dict, docs: list) -> str:
        import textwrap  # インポートが必要であればファイルの先頭に追加してください
        context = "\n\n".join(docs)
        ...
        return textwrap.dedent(f"""
            ...
            # 参考情報 (専門知識)
            以下は、計画書を作成するための根拠となるガイドラインや文献です。
            医学的な正確性を担保するために参照してください。ただし、**出力する文章には、これらの専門的な表現（例：早期離床、等尺性収縮運動、脱臼肢位など）をそのまま使わず、患者様が理解できる言葉に噛み砕いて**記述してください。
            ```text
            {context}
            ```
            ...
            # 作成指示
            上記の「参考情報」と「患者データ」を統合し、指定されたJSONスキーマに従って計画書を作成してください。
            - **最重要**: 生成する文章は、医療知識のない患者様やそのご家族が読んで即座に理解できるよう、**徹底して平易な言葉**で記述してください。
            - **言葉選びのルール**:
              - 「ROM訓練/可動域訓練」→「関節を動かす練習」
              - 「等尺性収縮」→「関節を動かさずに力を入れる運動」
              - 「ADL」→「日常生活の動作」
              - 「脱臼肢位」→「脱臼しやすい姿勢」
              - 「疼痛」→「痛み」
              - 「離床」→「ベッドから起き上がること」
            - **具体的な動作への変換**: 禁忌事項や注意点を記述する際は、「過度な屈曲禁止」のような抽象的な表現ではなく、「正座や横座りはしない」「深いソファーに座らない」「あぐらをかかない」など、**日常生活で避けるべき具体的な動作**を明記してください。
            - 患者データから判断して該当しない項目は「特記なし」としてください。
    ...
    def execute(self, patient_facts: dict):
        ...
        for i in range(len(final_docs)):
            final_contexts_with_metadata.append(
                {"content": final_docs[i], "metadata": final_metadatas[i] if i < len(final_metadatas) else {}}
            )

        final_prompt = self._construct_prompt(patient_facts, final_docs)

        # 個人情報保護のため、プロンプト全文は出力せず、IDのみ記録する
