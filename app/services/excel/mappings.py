...
# ---------------------------------------------------------
# 1. 基本テキスト・チェックボックスのマッピング
# ---------------------------------------------------------
# 通常のテキスト・チェックボックス項目のマッピング
# (日付の構成要素である year_txt 等は DATE_MAPPING で扱うため除外してもよいが、
#  既存のキー参照を維持するため、または個別の値書き込みに対応するため残すことも可能。
#  ここでは日付ロジックで上書きされる前提で定義を含めます)
TEXT_MAPPING = {
    # 主要なテキスト項目 (AI生成)
    "name": ("様式23_1", "F3"),
    "age": ("様式23_1", "AC3"),
...
# 日付項目のマッピング (キー名プレフィックス: (シート名, 年セル, 月セル, 日セル))
DATE_MAPPING = {
    "header_evaluation": ("様式23_1", "AN3", "AQ3", "AT3"),
    "header_onset": ("様式23_1", "AN4", "AQ4", "AT4"),
    "header_rehab_start": ("様式23_1", "AN5", "AQ5", "AT5"),
    "signature_explanation": ("様式23_1", "AP86", "AS86", "AU86"),
}

# 性別項目のマッピング
GENDER_MAPPING = {
    "male_value": "男",      # DB上の値（男性）
    "female_value": "女",    # DB上の値（女性）
    "male_cell": ("様式23_1", "V3"),   # 男性のセル座標
    "female_cell": ("様式23_1", "X3"), # 女性のセル座標
    "selected_mark": "㊚",   # 男性選択時のマーク（例）
    "selected_mark_f": "㊛", # 女性選択時のマーク
    "unselected_mark_m": "男",
    "unselected_mark_f": "女",
}

# 選択肢（ラジオボタン・複数チェック連動）のマッピング
# キー: DBのカラム名
# 値: { DBの値: [ (シート名, セル番地), ... ], ... }
# ※値に合致する場合、リスト内の全てのセルに "☑" を入れます。
SELECTION_MAPPING = {
    # 住宅種別
    "goal_p_residence_slct": {
        "home_detached": [("様式23_2", "E3"), ("様式23_2", "H3")],
        "home_apartment": [("様式23_2", "E3"), ("様式23_2", "K3")],
        "facility":       [("様式23_2", "O3")],
        "other":          [("様式23_2", "R3")],
    },
    # 復職の状況
    "goal_p_return_to_work_status_slct": {
        "current_job":  [("様式23_2", "E5")],
        "reassignment": [("様式23_2", "I5")],
        "new_job":      [("様式23_2", "M5")],
        "not_possible": [("様式23_2", "P5")],
        "other":        [("様式23_2", "S5")],
    },
    # 嚥下調整食の必要性
    ...
    # 栄養状態の評価
    "nutrition_status_assessment_slct": {
        "no_problem":        [("様式23_1", "J63")],
        "malnutrition":      [("様式23_1", "P63")],
        "malnutrition_risk": [("様式23_1", "V63")],
        "overnutrition":     [("様式23_1", "AC63")],
        "other":             [("様式23_1", "AJ63")],
    },
    # 不整脈の有無 (セル内の文字列を書き換えるパターン)
    # ここでは便宜上、値に対応する「書き込み文字列」を定義します
