...
class RehabPlanSchema(BaseModel):
    ...
    adl_equipment_and_assistance_details_txt: str = Field(
        description="FIM/BIの各項目点数に基づき、**現在**使用している福祉用具や、**現在**必要としている具体的な介助内容（現状の評価）を記述(1文あたり10文字から40文字程度、1～3文で構成。)(出力例: 整容:時間を要する 清拭:シャワー椅子使用 更衣:弾性ストッキング着脱に介助要 トイレ:手すり使用)"
    )

    goals_1_month_txt: str = Field(
        description="患者データ、特にADL状況や担当者所見から、1ヶ月で達成可能かつ具体的な短期目標（SMARTゴール）を設定(1文あたり10文字から40文字程度、1文で構成。)(出力例: ・除痛 ・歩行能力向上 ・ADL、IADL動作獲得 ・在宅復帰)"
    )
    goals_at_discharge_txt: str = Field(
        description="患者の全体像を考慮し、退院時に達成を目指す現実的な長期目標を設定(1文あたり10文字から40文字程度、1文で構成。)(出力例: ・屋内独歩自立 ・排泄動作の自立 ・復職 ・公共交通機関の利用)"
    )

    policy_treatment_txt: str = Field(
        description="全ての情報を統合し、リハビリテーションの全体的な治療方針を専門的に記述(1文あたり10文字から40文字程度、1～3文で構成。)(出力例: 疼痛コントロールを図りつつ、早期離床・ADL向上を目指す。)"
    )
    policy_content_txt: str = Field(
        description="治療方針に基づき、理学療法・作業療法・言語聴覚療法の具体的な訓練メニュー案を箇条書き形式で複数提案。作業療法や理学療法など、職種が変わるときのみ改行してください。(1行あたり10文字から40文字程度、1～3行で構成。)(出力例: ・関節可動域訓練 ・筋力増強訓練 ・歩行訓練 ・ADL動作練習)"
    )
    ...
    goal_a_action_plan_txt: str = Field(
        description="設定した活動目標（ADLなど）を達成するための具体的な対応方針、環境調整、指導内容を記述(1文あたり10文字から40文字程度、1～3文で構成。)(出力例: ・病棟内歩行の励行 ・セルフケアの自立促進 ・転倒予防の指導)"
    )
    goal_s_env_action_plan_txt: str = Field(
        description="退院後の生活を見据え、必要と考えられる住宅改修、社会資源の活用（介護保険サービス、障害福祉サービス等）に関する具体的な対応方針を記述(1文あたり10文字から40文字程度、1～3文で構成。)(出力例: ・手すりの設置提案 ・介護保険サービスの調整 ・家屋調査の実施)"
    )
    goal_p_action_plan_txt: str = Field(
        description="参加目標（復職、就学、家庭内役割など）を達成するための具体的な対応方針、関連機関との連携、家族への指導内容などを記述(1文あたり10文字から40文字程度、1～3文で構成。)(出力例: ・復職に向けた通勤練習 ・家族への介助方法指導 ・役割活動の提案)"
    )
    goal_s_psychological_action_plan_txt: str = Field(
        description="心理面での目標（障害受容、精神的支援など）に対する具体的な関わり方、声かけ、家族への説明内容などを記述(1文あたり10文字から40文字程度、1～2文で構成。)(出力例: ・傾聴と受容的態度 ・不安の軽減 ・意欲向上のための声かけ)"
    )
    goal_s_3rd_party_action_plan_txt: str = Field(
        description="主介護者や家族の負担軽減、環境の変化に対する具体的な支援策や社会資源の活用提案などを記述(1文あたり10文字から40文字程度、1～2文で構成。)(出力例: ・レスパイトケアの活用 ・介護者への心理的サポート ・負担軽減のための福祉用具導入)"
    )
...
class PatientMasterSchema(BaseModel):
    ...
    # --- Patientモデル由来の項目 ---
    name: Optional[str] = Field(None, description="患者氏名") # 無し
    age: Optional[int] = Field(None, description="患者の年齢") # Q
    gender: Optional[str] = Field(None, description="患者の性別。'男'または'女'") # EW

    # --- RehabilitationPlanモデル由来の項目 ---
    # 【1枚目】
    header_evaluation_date: Optional[date] = Field(None, description="評価日 (YYYY-MM-DD形式)") # R
    header_disease_name_txt: Optional[str] = Field(None, description="算定病名") # Z
    header_treatment_details_txt: Optional[str] = Field(None, description="治療内容（手術名など）") # ER
    header_onset_date: Optional[date] = Field(None, description="発症日または手術日 (YYYY-MM-DD形式)") # AC
    header_rehab_start_date: Optional[date] = Field(None, description="リハビリテーション開始日 (YYYY-MM-DD形式)") # AD
    header_therapy_pt_chk: Optional[bool] = Field(None, description="理学療法(PT)の実施有無") # N
    header_therapy_ot_chk: Optional[bool] = Field(None, description="作業療法(OT)の実施有無") # O
    header_therapy_st_chk: Optional[bool] = Field(None, description="言語聴覚療法(ST)の実施有無") # P
    main_comorbidities_txt: Optional[str] = Field(None, description="併存疾患・合併症") # AE
    main_risks_txt: Optional[str] = Field(None, description="安静度やリハビリテーション施行上のリスク") # AF
    main_contraindications_txt: Optional[str] = Field(None, description="禁忌や医学的な特記事項・注意点") # AG
    func_consciousness_disorder_chk: Optional[bool] = Field(None, description="意識障害の有無") # AI
    func_consciousness_disorder_jcs_gcs_txt: Optional[str] = Field(None, description="意識レベル (JCS, GCS)") # AH
    ...
    func_risk_hypertension_chk: Optional[bool] = Field(None, description="高血圧症の有無") # AJ
    func_risk_dyslipidemia_chk: Optional[bool] = Field(None, description="脂質異常症の有無") # AK
    func_risk_diabetes_chk: Optional[bool] = Field(None, description="糖尿病の有無") # AL
    ...
    func_risk_ckd_chk: Optional[bool] = Field(None, description="慢性腎臓病(CKD)の有無") # AM
    func_risk_family_history_chk: Optional[bool] = Field(None, description="家族歴の有無") # なし
    func_risk_angina_chk: Optional[bool] = Field(None, description="狭心症の有無") # AN
    func_risk_omi_chk: Optional[bool] = Field(None, description="陳旧性心筋梗塞(OMI)の有無") # AO
    func_risk_other_chk: Optional[bool] = Field(None, description="その他の危険因子の有無") # AP
    func_risk_other_txt: Optional[str] = Field(None, description="その他の危険因子の詳細") # BK
    func_swallowing_disorder_chk: Optional[bool] = Field(None, description="摂食嚥下障害の有無") # BF
    func_swallowing_disorder_txt: Optional[str] = Field(None, description="摂食嚥下障害の詳細") # DM
    ...
    func_pain_chk: Optional[bool] = Field(None, description="疼痛の有無") # BI
    func_pain_txt: Optional[str] = Field(None, description="疼痛の詳細 (部位、程度、NRSなど)") # DP
    func_other_chk: Optional[bool] = Field(None, description="その他の心身機能障害の有無") # BJ
    func_other_txt: Optional[str] = Field(None, description="その他の心身機能障害の詳細") # DQ
    func_rom_limitation_chk: Optional[bool] = Field(None, description="関節可動域制限の有無") # BA
    func_rom_limitation_txt: Optional[str] = Field(None, description="関節可動域制限の詳細") # DL
    ...
    func_muscle_weakness_chk: Optional[bool] = Field(None, description="筋力低下の有無") # AZ
    func_muscle_weakness_txt: Optional[str] = Field(None, description="筋力低下の詳細 (MMTなど)") # DK
    func_motor_dysfunction_chk: Optional[bool] = Field(None, description="運動機能障害の有無") # なし
    func_motor_paralysis_chk: Optional[bool] = Field(None, description="麻痺の有無") # AQ
    func_motor_involuntary_movement_chk: Optional[bool] = Field(None, description="不随意運動の有無") # AR
    func_motor_ataxia_chk: Optional[bool] = Field(None, description="運動失調の有無") # AS
    func_motor_parkinsonism_chk: Optional[bool] = Field(None, description="パーキンソニズムの有無") # AT
    ...
    func_sensory_hearing_chk: Optional[bool] = Field(None, description="聴覚障害の有無") # AV
    func_sensory_vision_chk: Optional[bool] = Field(None, description="視覚障害の有無") # AW
    func_sensory_superficial_chk: Optional[bool] = Field(None, description="表在感覚障害の有無") # AX
    func_sensory_deep_chk: Optional[bool] = Field(None, description="深部感覚障害の有無") # AY
    func_speech_disorder_chk: Optional[bool] = Field(None, description="音声発話障害の有無") # BB
    func_speech_articulation_chk: Optional[bool] = Field(None, description="構音障害の有無") # BC
    func_speech_aphasia_chk: Optional[bool] = Field(None, description="失語症の有無") # BD
    func_speech_stuttering_chk: Optional[bool] = Field(None, description="吃音の有無") # なし
    func_speech_other_chk: Optional[bool] = Field(None, description="その他の音声発話障害の有無") # BE
    ...
    func_behavioral_psychiatric_disorder_chk: Optional[bool] = Field(None, description="精神行動障害の有無") # BH
    func_behavioral_psychiatric_disorder_txt: Optional[str] = Field(None, description="精神行動障害の詳細") # DO
    ...
    func_basic_getting_up_chk: Optional[bool] = Field(None, description="起き上がり動作の評価有無") # FJ
    func_basic_getting_up_independent_chk: Optional[bool] = Field(None, description="起き上がり: 自立") # 無し
    func_basic_getting_up_partial_assistance_chk: Optional[bool] = Field(None, description="起き上がり: 一部介助") # 無し
    func_basic_getting_up_assistance_chk: Optional[bool] = Field(None, description="起き上がり: 全介助") # 無し
    func_basic_getting_up_not_performed_chk: Optional[bool] = Field(None, description="起き上がり: 行わない") # 無し
    func_basic_standing_up_chk: Optional[bool] = Field(None, description="立ち上がり動作の評価有無") # FK
    func_basic_standing_up_independent_chk: Optional[bool] = Field(None, description="立ち上がり: 自立") # 無し
    func_basic_standing_up_partial_assistance_chk: Optional[bool] = Field(None, description="立ち上がり: 一部介助") # 無し
    func_basic_standing_up_assistance_chk: Optional[bool] = Field(None, description="立ち上がり: 全介助") # 無し
    func_basic_standing_up_not_performed_chk: Optional[bool] = Field(None, description="立ち上がり: 行わない") # 無し
    func_basic_sitting_balance_chk: Optional[bool] = Field(None, description="座位保持の評価有無") # FL
    func_basic_sitting_balance_independent_chk: Optional[bool] = Field(None, description="座位保持: 自立") # 無し
    func_basic_sitting_balance_partial_assistance_chk: Optional[bool] = Field(None, description="座位保持: 一部介助") # 無し
    func_basic_sitting_balance_assistance_chk: Optional[bool] = Field(None, description="座位保持: 全介助") # 無し
    func_basic_sitting_balance_not_performed_chk: Optional[bool] = Field(None, description="座位保持: 行わない") # 無し
    func_basic_standing_balance_chk: Optional[bool] = Field(None, description="立位保持の評価有無") # FM
    func_basic_standing_balance_independent_chk: Optional[bool] = Field(None, description="立位保持: 自立") # 無し
    func_basic_standing_balance_partial_assistance_chk: Optional[bool] = Field(None, description="立位保持: 一部介助") # 無し
    func_basic_standing_balance_assistance_chk: Optional[bool] = Field(None, description="立位保持: 全介助") # 無し
    ...
    adl_eating_fim_start_val: Optional[int] = Field(None, description="食事のFIM開始時値") # BR
    adl_eating_fim_current_val: Optional[int] = Field(None, description="食事のFIM現在値") # BS
    adl_eating_bi_start_val: Optional[int] = Field(None, description="食事のBI開始時値") # 無し
    adl_eating_bi_current_val: Optional[int] = Field(None, description="食事のBI現在値") # 無し
    adl_grooming_fim_start_val: Optional[int] = Field(None, description="整容のFIM開始時値") # BT
    adl_grooming_fim_current_val: Optional[int] = Field(None, description="整容のFIM現在値") # BU
    adl_grooming_bi_start_val: Optional[int] = Field(None, description="整容のBI開始時値") # 無し
    adl_grooming_bi_current_val: Optional[int] = Field(None, description="整容のBI現在値") # 無し
    ...
    adl_bathing_bi_start_val: Optional[int] = Field(None, description="入浴のBI開始時値") # 無し
    adl_bathing_bi_current_val: Optional[int] = Field(None, description="入浴のBI現在値") # 無し
    adl_dressing_upper_fim_start_val: Optional[int] = Field(None, description="更衣(上半身)のFIM開始時値") # BX
    adl_dressing_upper_fim_current_val: Optional[int] = Field(None, description="更衣(上半身)のFIM現在値") # BY
    adl_dressing_lower_fim_start_val: Optional[int] = Field(None, description="更衣(下半身)のFIM開始時値") # BZ
    adl_dressing_lower_fim_current_val: Optional[int] = Field(None, description="更衣(下半身)のFIM現在値") # CA
    adl_dressing_bi_start_val: Optional[int] = Field(None, description="更衣のBI開始時値") # 無し
    adl_dressing_bi_current_val: Optional[int] = Field(None, description="更衣のBI現在値") # 無し
    adl_toileting_fim_start_val: Optional[int] = Field(None, description="トイレ動作のFIM開始時値") # CB
    adl_toileting_fim_current_val: Optional[int] = Field(None, description="トイレ動作のFIM現在値") # CC
    adl_toileting_bi_start_val: Optional[int] = Field(None, description="トイレ動作のBI開始時値") # 無し
    adl_toileting_bi_current_val: Optional[int] = Field(None, description="トイレ動作のBI現在値") # 無し
    adl_bladder_management_fim_start_val: Optional[int] = Field(None, description="排尿管理のFIM開始時値") # CD
    adl_bladder_management_fim_current_val: Optional[int] = Field(None, description="排尿管理のFIM現在値") # CE
    adl_bladder_management_bi_start_val: Optional[int] = Field(None, description="排尿管理のBI開始時値") # 無し
    adl_bladder_management_bi_current_val: Optional[int] = Field(None, description="排尿管理のBI現在値") # 無し
    adl_bowel_management_fim_start_val: Optional[int] = Field(None, description="排便管理のFIM開始時値") # CF
    adl_bowel_management_fim_current_val: Optional[int] = Field(None, description="排便管理のFIM現在値") # CG
    adl_bowel_management_bi_start_val: Optional[int] = Field(None, description="排便管理のBI開始時値") # 無し
    adl_bowel_management_bi_current_val: Optional[int] = Field(None, description="排便管理のBI現在値") # 無し
    adl_transfer_bed_chair_wc_fim_start_val: Optional[int] = Field(None, description="移乗(ベッド・椅子・車椅子)のFIM開始時値") # CH
    adl_transfer_bed_chair_wc_fim_current_val: Optional[int] = Field(None, description="移乗(ベッド・椅子・車椅子)のFIM現在値") # CI
    adl_transfer_toilet_fim_start_val: Optional[int] = Field(None, description="移乗(トイレ)のFIM開始時値") # CJ
    adl_transfer_toilet_fim_current_val: Optional[int] = Field(None, description="移乗(トイレ)のFIM現在値") # CK
    adl_transfer_tub_shower_fim_start_val: Optional[int] = Field(None, description="移乗(浴槽・シャワー)のFIM開始時値") # CL
    adl_transfer_tub_shower_fim_current_val: Optional[int] = Field(None, description="移乗(浴槽・シャワー)のFIM現在値") # CM
    adl_transfer_bi_start_val: Optional[int] = Field(None, description="移乗のBI開始時値") # 無し
    adl_transfer_bi_current_val: Optional[int] = Field(None, description="移乗のBI現在値") # 無し
    adl_locomotion_walk_walkingAids_wc_fim_start_val: Optional[int] = Field(None, description="移動(歩行/車椅子)のFIM開始時値") # CN
    adl_locomotion_walk_walkingAids_wc_fim_current_val: Optional[int] = Field(None, description="移動(歩行/車椅子)のFIM現在値") # CO
    adl_locomotion_walk_walkingAids_wc_bi_start_val: Optional[int] = Field(None, description="移動のBI開始時値") # 無し
    adl_locomotion_walk_walkingAids_wc_bi_current_val: Optional[int] = Field(None, description="移動のBI現在値") # 無し
    adl_locomotion_stairs_fim_start_val: Optional[int] = Field(None, description="階段のFIM開始時値") # CP
    adl_locomotion_stairs_fim_current_val: Optional[int] = Field(None, description="階段のFIM現在値") # CQ
    adl_locomotion_stairs_bi_start_val: Optional[int] = Field(None, description="階段のBI開始時値") # 無し
    adl_locomotion_stairs_bi_current_val: Optional[int] = Field(None, description="階段のBI現在値") # 無し
    adl_comprehension_fim_start_val: Optional[int] = Field(None, description="理解のFIM開始時値") # CW
    adl_comprehension_fim_current_val: Optional[int] = Field(None, description="理解のFIM現在値") # CV
    adl_expression_fim_start_val: Optional[int] = Field(None, description="表出のFIM開始時値") # DR
    adl_expression_fim_current_val: Optional[int] = Field(None, description="表出のFIM現在値") # CY
    adl_social_interaction_fim_start_val: Optional[int] = Field(None, description="社会的交流のFIM開始時値") # CZ
    adl_social_interaction_fim_current_val: Optional[int] = Field(None, description="社会的交流のFIM現在値") # DA
    adl_problem_solving_fim_start_val: Optional[int] = Field(None, description="問題解決のFIM開始時値") # DB
    adl_problem_solving_fim_current_val: Optional[int] = Field(None, description="問題解決のFIM現在値") # DC
    adl_memory_fim_start_val: Optional[int] = Field(None, description="記憶のFIM開始時値") # DD
    adl_memory_fim_current_val: Optional[int] = Field(None, description="記憶のFIM現在値") # DE
    adl_equipment_and_assistance_details_txt: Optional[str] = Field(None, description="ADLの補装具・介助方法の詳細") # DJ
    nutrition_height_chk: Optional[bool] = Field(None, description="身長測定の有無") # 無し
    nutrition_height_val: Optional[float] = Field(None, description="身長 (cm)") # 無し
    nutrition_weight_chk: Optional[bool] = Field(None, description="体重測定の有無") # 無し
    nutrition_weight_val: Optional[float] = Field(None, description="体重 (kg)") # 無し
    nutrition_bmi_chk: Optional[bool] = Field(None, description="BMI計算の有無") # 無し
    nutrition_bmi_val: Optional[float] = Field(None, description="BMI値") # EU
    nutrition_method_oral_chk: Optional[bool] = Field(None, description="栄養補給(経口)の有無") # DS
    nutrition_method_oral_meal_chk: Optional[bool] = Field(None, description="経口栄養(食事)の有無") # DT
    nutrition_method_oral_supplement_chk: Optional[bool] = Field(None, description="経口栄養(補助食品)の有無") # DU
    nutrition_method_tube_chk: Optional[bool] = Field(None, description="経管栄養の有無") # DV
    nutrition_method_iv_chk: Optional[bool] = Field(None, description="静脈栄養の有無") # DW
    nutrition_method_iv_peripheral_chk: Optional[bool] = Field(None, description="末梢静脈栄養の有無") # DX
    nutrition_method_iv_central_chk: Optional[bool] = Field(None, description="中心静脈栄養の有無") # DY
    nutrition_method_peg_chk: Optional[bool] = Field(None, description="胃ろうの有無") # 無し
    nutrition_swallowing_diet_slct: Optional[str] = Field(None, description="嚥下調整食の選択") # データセットには有・無でそれぞれ項目が存在
    nutrition_swallowing_diet_code_txt: Optional[str] = Field(None, description="嚥下調整食コード") # EB
    nutrition_status_assessment_slct: Optional[str] = Field(None, description="栄養状態評価の選択") # 無し
    nutrition_status_assessment_other_txt: Optional[str] = Field(None, description="その他の栄養状態評価") # 無し
    nutrition_required_energy_val: Optional[int] = Field(None, description="必要エネルギー量 (kcal)") # EC
    nutrition_required_protein_val: Optional[int] = Field(None, description="必要タンパク質量 (g)") # ED
    nutrition_total_intake_energy_val: Optional[int] = Field(None, description="総摂取エネルギー量 (kcal)") # EE
    nutrition_total_intake_protein_val: Optional[int] = Field(None, description="総摂取タンパク質量 (g)") # EF
    social_care_level_status_chk: Optional[bool] = Field(None, description="介護保険状況の有無") # 無し
    social_care_level_applying_chk: Optional[bool] = Field(None, description="介護保険(申請中)の有無") # 無し
    social_care_level_support_chk: Optional[bool] = Field(None, description="要支援認定の有無") # 無し
    social_care_level_support_num1_slct: Optional[bool] = Field(None, description="要支援1") # 無し
    social_care_level_support_num2_slct: Optional[bool] = Field(None, description="要支援2") # 無し
    social_care_level_care_slct: Optional[bool] = Field(None, description="要介護認定の有無") # データセットでは有・無・非該当・区分で入力されている
    social_care_level_care_num1_slct: Optional[bool] = Field(None, description="要介護1") # 無し
    social_care_level_care_num2_slct: Optional[bool] = Field(None, description="要介護2") # 無し
    social_care_level_care_num3_slct: Optional[bool] = Field(None, description="要介護3") # 無し
    social_care_level_care_num4_slct: Optional[bool] = Field(None, description="要介護4") # 無し
    social_care_level_care_num5_slct: Optional[bool] = Field(None, description="要介護5") # 無し
    social_disability_certificate_physical_chk: Optional[bool] = Field(None, description="身体障害者手帳の有無") # 無し
    social_disability_certificate_physical_txt: Optional[str] = Field(None, description="身体障害者手帳の詳細") # IT
    social_disability_certificate_physical_type_txt: Optional[str] = Field(None, description="身体障害者手帳の種別") # 無し
    social_disability_certificate_physical_rank_val: Optional[int] = Field(None, description="身体障害者手帳の等級") # EH
    social_disability_certificate_mental_chk: Optional[bool] = Field(None, description="精神障害者保健福祉手帳の有無") # 無し
    social_disability_certificate_mental_rank_val: Optional[int] = Field(None, description="精神障害者保健福祉手帳の等級") # 無し
    social_disability_certificate_intellectual_chk: Optional[bool] = Field(None, description="療育手帳の有無") # 無し
    social_disability_certificate_intellectual_txt: Optional[str] = Field(None, description="療育手帳の詳細") # 無し
    social_disability_certificate_intellectual_grade_txt: Optional[str] = Field(None, description="療育手帳の等級") # 無し
    social_disability_certificate_other_chk: Optional[bool] = Field(None, description="その他の手帳の有無") # 無し
    social_disability_certificate_other_txt: Optional[str] = Field(None, description="その他の手帳の詳細") # データセットでは「その他難病」と定義されている(入力例：精神保健福祉手帳2級所持)
    goals_1_month_txt: Optional[str] = Field(None, description="1ヶ月の短期目標") # EL
    goals_at_discharge_txt: Optional[str] = Field(None, description="退院時の長期目標") # データセットでは「目標終了時」と定義されている
    goals_planned_hospitalization_period_chk: Optional[bool] = Field(None, description="入院期間の予定有無") # 無し
    goals_planned_hospitalization_period_txt: Optional[str] = Field(None, description="入院期間の予定詳細") # データセットでは「予定入院期間」と定義されている
    goals_discharge_destination_chk: Optional[bool] = Field(None, description="退院先の予定有無") # 無し
    goals_discharge_destination_txt: Optional[str] = Field(None, description="退院先の予定詳細") # データセットでは「退院先」と定義されている
    goals_long_term_care_needed_chk: Optional[bool] = Field(None, description="長期療養の必要性有無") # EP
    policy_treatment_txt: Optional[str] = Field(None, description="リハビリテーションの全体的な治療方針") # EQ
    policy_content_txt: Optional[str] = Field(None, description="具体的な訓練メニュー案") # 無し

    # 署名ではなく製造者表記のような形
    signature_rehab_doctor_txt: Optional[str] = Field(None, description="リハビリテーション科医師の署名") # 無し
    signature_primary_doctor_txt: Optional[str] = Field(None, description="主治医の署名") # S
    signature_pt_txt: Optional[str] = Field(None, description="理学療法士の署名") # X
    signature_ot_txt: Optional[str] = Field(None, description="作業療法士の署名") # CX
    signature_st_txt: Optional[str] = Field(None, description="言語聴覚士の署名") # Y
    signature_nurse_txt: Optional[str] = Field(None, description="看護師の署名") # T
    signature_dietitian_txt: Optional[str] = Field(None, description="管理栄養士の署名") # V
    signature_social_worker_txt: Optional[str] = Field(None, description="ソーシャルワーカーの署名") # 無し
    signature_explained_to_txt: Optional[str] = Field(None, description="説明を受けた方の氏名") # 無し
    signature_explanation_date: Optional[date] = Field(None, description="説明日 (YYYY-MM-DD形式)") # 無し
    signature_explainer_txt: Optional[str] = Field(None, description="説明者の氏名") # 無し

    # 【2枚目】
    goal_p_residence_chk: Optional[bool] = Field(None, description="住居場所の目標有無") # 無し
    goal_p_residence_slct: Optional[str] = Field(None, description="住居場所の選択 (自宅など)") # データセットでは「(目標・環境)自宅・マンション・施設」等と定義されている
    goal_p_residence_other_txt: Optional[str] = Field(None, description="その他の住居場所") # II
    goal_p_return_to_work_chk: Optional[bool] = Field(None, description="復職の目標有無") # 無し
    goal_p_return_to_work_status_slct: Optional[str] = Field(None, description="復職状況の選択") # 無し
    goal_p_return_to_work_status_other_txt: Optional[str] = Field(None, description="その他の復職状況") # FC
    goal_p_return_to_work_commute_change_chk: Optional[bool] = Field(None, description="通勤方法の変更有無") # 無し
    goal_p_schooling_chk: Optional[bool] = Field(None, description="就学の目標有無") # 無し
    goal_p_schooling_status_possible_chk: Optional[bool] = Field(None, description="就学: 可能") # FD
    goal_p_schooling_status_needs_consideration_chk: Optional[bool] = Field(None, description="就学: 要検討") # FE
    goal_p_schooling_status_change_course_chk: Optional[bool] = Field(None, description="就学: 転科・転校") # FF
    goal_p_schooling_status_not_possible_chk: Optional[bool] = Field(None, description="就学: 不可能") # 無し
    goal_p_schooling_status_other_chk: Optional[bool] = Field(None, description="就学: その他") # FG
    goal_p_schooling_status_other_txt: Optional[str] = Field(None, description="その他の就学状況") # FH
    goal_p_schooling_destination_chk: Optional[bool] = Field(None, description="就学先の有無") # 無し
    goal_p_schooling_destination_txt: Optional[str] = Field(None, description="就学先の詳細") # 無し
    goal_p_schooling_commute_change_chk: Optional[bool] = Field(None, description="通学方法の変更有無") # 無し
    goal_p_schooling_commute_change_txt: Optional[str] = Field(None, description="通学方法の変更詳細") # 無し
    goal_p_household_role_chk: Optional[bool] = Field(None, description="家庭内役割の目標有無") # 無し
    goal_p_household_role_txt: Optional[str] = Field(None, description="家庭内役割の詳細") # データセットでは「家庭内役割」と定義されている
    goal_p_social_activity_chk: Optional[bool] = Field(None, description="社会活動の目標有無") # 無し
    goal_p_social_activity_txt: Optional[str] = Field(None, description="社会活動の詳細") # データセットでは「社会活動」と定義されている
    goal_p_hobby_chk: Optional[bool] = Field(None, description="趣味の目標有無") # 無し
    goal_p_hobby_txt: Optional[str] = Field(None, description="趣味の詳細") # データセットでは「趣味活動」と定義されている
    goal_a_action_plan_txt: Optional[str] = Field(None, description="活動目標の達成に向けた具体的対応方針") # データセットでは「活動_基本_具体的内容」と定義されている
    goal_s_env_action_plan_txt: Optional[str] = Field(None, description="環境因子への具体的対応方針") # 無し
    goal_p_action_plan_txt: Optional[str] = Field(None, description="参加目標の達成に向けた具体的対応方針") # データセットでは「参加_具体的対応」と定義されている
    goal_s_psychological_action_plan_txt: Optional[str] = Field(None, description="心理面への具体的対応方針") # 無し
    goal_s_3rd_party_action_plan_txt: Optional[str] = Field(None, description="人的因子への具体的対応方針") # 無し

    # --- 2枚目 goal_a_* (活動目標) ---
    goal_a_bed_mobility_chk: Optional[bool] = Field(None, description="活動目標(床上移動)の有無") # 無し
    goal_a_bed_mobility_independent_chk: Optional[bool] = Field(None, description="活動目標(床上移動): 自立") # 無し
    goal_a_bed_mobility_assistance_chk: Optional[bool] = Field(None, description="活動目標(床上移動): 介助") # 無し
    goal_a_bed_mobility_not_performed_chk: Optional[bool] = Field(None, description="活動目標(床上移動): 行わない") # 無し
    goal_a_bed_mobility_equipment_chk: Optional[bool] = Field(None, description="活動目標(床上移動): 補装具") # 無し
    goal_a_bed_mobility_environment_setup_chk: Optional[bool] = Field(None, description="活動目標(床上移動): 環境設定") # 無し
    goal_a_indoor_mobility_chk: Optional[bool] = Field(None, description="活動目標(屋内移動)の有無") # 無し
    goal_a_indoor_mobility_independent_chk: Optional[bool] = Field(None, description="活動目標(屋内移動): 自立") # 無し
    goal_a_indoor_mobility_assistance_chk: Optional[bool] = Field(None, description="活動目標(屋内移動): 介助") # 無し
    goal_a_indoor_mobility_not_performed_chk: Optional[bool] = Field(None, description="活動目標(屋内移動): 行わない") # 無し
    goal_a_indoor_mobility_equipment_chk: Optional[bool] = Field(None, description="活動目標(屋内移動): 補装具") # 無し
    goal_a_indoor_mobility_equipment_txt: Optional[str] = Field(None, description="活動目標(屋内移動): 補装具詳細") # 無し
    goal_a_outdoor_mobility_chk: Optional[bool] = Field(None, description="活動目標(屋外移動)の有無") # 無し
    goal_a_outdoor_mobility_independent_chk: Optional[bool] = Field(None, description="活動目標(屋外移動): 自立") # 無し
    goal_a_outdoor_mobility_assistance_chk: Optional[bool] = Field(None, description="活動目標(屋外移動): 介助") # 無し
    goal_a_outdoor_mobility_not_performed_chk: Optional[bool] = Field(None, description="活動目標(屋外移動): 行わない") # 無し
    goal_a_outdoor_mobility_equipment_chk: Optional[bool] = Field(None, description="活動目標(屋外移動): 補装具") # 無し
    goal_a_outdoor_mobility_equipment_txt: Optional[str] = Field(None, description="活動目標(屋外移動): 補装具詳細") # 無し
    goal_a_driving_chk: Optional[bool] = Field(None, description="活動目標(自動車運転)の有無") # 無し
    goal_a_driving_independent_chk: Optional[bool] = Field(None, description="活動目標(自動車運転): 自立") # GL
    goal_a_driving_assistance_chk: Optional[bool] = Field(None, description="活動目標(自動車運転): 介助") # GM
    goal_a_driving_not_performed_chk: Optional[bool] = Field(None, description="活動目標(自動車運転): 行わない") # 無し
    goal_a_driving_modification_chk: Optional[bool] = Field(None, description="活動目標(自動車運転): 改造") # 無し
    goal_a_driving_modification_txt: Optional[str] = Field(None, description="活動目標(自動車運転): 改造詳細") # 無し
    goal_a_public_transport_chk: Optional[bool] = Field(None, description="活動目標(公共交通機関)の有無") # 無し
    goal_a_public_transport_independent_chk: Optional[bool] = Field(None, description="活動目標(公共交通機関): 自立") # GN
    goal_a_public_transport_assistance_chk: Optional[bool] = Field(None, description="活動目標(公共交通機関): 介助") # GO
    goal_a_public_transport_not_performed_chk: Optional[bool] = Field(None, description="活動目標(公共交通機関): 行わない") # 無し
    goal_a_public_transport_type_chk: Optional[bool] = Field(None, description="活動目標(公共交通機関): 種類") # 無し
    goal_a_public_transport_type_txt: Optional[str] = Field(None, description="活動目標(公共交通機関): 種類詳細") # 無し
    goal_a_toileting_chk: Optional[bool] = Field(None, description="活動目標(排泄)の有無") # FS
    goal_a_toileting_independent_chk: Optional[bool] = Field(None, description="活動目標(排泄): 自立") # GP
    goal_a_toileting_assistance_chk: Optional[bool] = Field(None, description="活動目標(排泄): 介助") # GQ
    goal_a_toileting_assistance_clothing_chk: Optional[bool] = Field(None, description="活動目標(排泄): 衣服の着脱") # データセットでは「(目標・活動・排泄)下衣操作」と定義されている
    goal_a_toileting_assistance_wiping_chk: Optional[bool] = Field(None, description="活動目標(排泄): 清拭") # データセットでは「(目標・活動・排泄)拭き動作」と定義されている
    goal_a_toileting_assistance_catheter_chk: Optional[bool] = Field(None, description="活動目標(排泄): カテーテル") # データセットでは「(目標・活動・排泄)カテーテル」と定義されている
    goal_a_toileting_type_chk: Optional[bool] = Field(None, description="活動目標(排泄): 種類") # 無し
    goal_a_toileting_type_western_chk: Optional[bool] = Field(None, description="活動目標(排泄): 洋式") # GU
    goal_a_toileting_type_japanese_chk: Optional[bool] = Field(None, description="活動目標(排泄): 和式") # GV
    goal_a_toileting_type_other_chk: Optional[bool] = Field(None, description="活動目標(排泄): その他") # GW
    goal_a_toileting_type_other_txt: Optional[str] = Field(None, description="活動目標(排泄): その他詳細") # GX
    goal_a_eating_chk: Optional[bool] = Field(None, description="活動目標(食事)の有無") # 無し
    goal_a_eating_independent_chk: Optional[bool] = Field(None, description="活動目標(食事): 自立") # GY
    goal_a_eating_assistance_chk: Optional[bool] = Field(None, description="活動目標(食事): 介助") # GZ
    goal_a_eating_not_performed_chk: Optional[bool] = Field(None, description="活動目標(食事): 行わない") # 無し
    goal_a_eating_method_chopsticks_chk: Optional[bool] = Field(None, description="活動目標(食事): 箸") # HB
    goal_a_eating_method_fork_etc_chk: Optional[bool] = Field(None, description="活動目標(食事): フォーク等") # HC
    goal_a_eating_method_tube_feeding_chk: Optional[bool] = Field(None, description="活動目標(食事): 経管栄養") # HA
    goal_a_eating_diet_form_txt: Optional[str] = Field(None, description="活動目標(食事): 食事形態") # HE
    goal_a_grooming_chk: Optional[bool] = Field(None, description="活動目標(整容)の有無") # 無し
    goal_a_grooming_independent_chk: Optional[bool] = Field(None, description="活動目標(整容): 自立") # HF
    goal_a_grooming_assistance_chk: Optional[bool] = Field(None, description="活動目標(整容): 介助") # HG
    goal_a_dressing_chk: Optional[bool] = Field(None, description="活動目標(更衣)の有無") # 無し
    goal_a_dressing_independent_chk: Optional[bool] = Field(None, description="活動目標(更衣): 自立") # HH
    goal_a_dressing_assistance_chk: Optional[bool] = Field(None, description="活動目標(更衣): 介助") # HI
    goal_a_bathing_chk: Optional[bool] = Field(None, description="活動目標(入浴)の有無") # FT
    goal_a_bathing_independent_chk: Optional[bool] = Field(None, description="活動目標(入浴): 自立") # HJ
    goal_a_bathing_assistance_chk: Optional[bool] = Field(None, description="活動目標(入浴): 介助") # HK
    goal_a_bathing_type_tub_chk: Optional[bool] = Field(None, description="活動目標(入浴): 浴槽") # HL
    goal_a_bathing_type_shower_chk: Optional[bool] = Field(None, description="活動目標(入浴): シャワー") # HM
    goal_a_bathing_assistance_body_washing_chk: Optional[bool] = Field(None, description="活動目標(入浴): 洗身") # HN
    goal_a_bathing_assistance_transfer_chk: Optional[bool] = Field(None, description="活動目標(入浴): 移乗") # HO
    goal_a_housework_meal_chk: Optional[bool] = Field(None, description="活動目標(家事)の有無") # 無し
    goal_a_housework_meal_all_chk: Optional[bool] = Field(None, description="活動目標(家事): 全般") # HP
    goal_a_housework_meal_not_performed_chk: Optional[bool] = Field(None, description="活動目標(家事): 行わない") # 無し
    goal_a_housework_meal_partial_chk: Optional[bool] = Field(None, description="活動目標(家事): 一部") # HQ
    goal_a_housework_meal_partial_txt: Optional[str] = Field(None, description="活動目標(家事): 一部詳細") # HR
    goal_a_writing_chk: Optional[bool] = Field(None, description="活動目標(書字)の有無") # 無し
    goal_a_writing_independent_chk: Optional[bool] = Field(None, description="活動目標(書字): 自立") # HS
    goal_a_writing_independent_after_hand_change_chk: Optional[bool] = Field(None, description="活動目標(書字): 利き手交換後自立") # HT
    goal_a_writing_other_chk: Optional[bool] = Field(None, description="活動目標(書字): その他") # HU
    goal_a_writing_other_txt: Optional[str] = Field(None, description="活動目標(書字): その他詳細") # HV
    goal_a_ict_chk: Optional[bool] = Field(None, description="活動目標(ICT機器)の有無") # 無し
    goal_a_ict_independent_chk: Optional[bool] = Field(None, description="活動目標(ICT機器): 自立") # HW
    goal_a_ict_assistance_chk: Optional[bool] = Field(None, description="活動目標(ICT機器): 介助") # HX
    goal_a_communication_chk: Optional[bool] = Field(None, description="活動目標(コミュニケーション)の有無") # 無し
    goal_a_communication_independent_chk: Optional[bool] = Field(None, description="活動目標(コミュニケーション): 自立") # HY
    goal_a_communication_assistance_chk: Optional[bool] = Field(None, description="活動目標(コミュニケーション): 介助") # HZ
    goal_a_communication_device_chk: Optional[bool] = Field(None, description="活動目標(コミュニケーション): 機器使用") # 無し
    goal_a_communication_letter_board_chk: Optional[bool] = Field(None, description="活動目標(コミュニケーション): 文字盤使用") # IA
    goal_a_communication_cooperation_chk: Optional[bool] = Field(None, description="活動目標(コミュニケーション): 協力") # IB

    # --- 2枚目 goal_s_* (対応を要する項目) ---
    goal_s_psychological_support_chk: Optional[bool] = Field(None, description="対応項目(心理的支援)の有無") # IC
    goal_s_psychological_support_txt: Optional[str] = Field(None, description="対応項目(心理的支援): 詳細") # 無し
    goal_s_disability_acceptance_chk: Optional[bool] = Field(None, description="対応項目(障害受容)の有無") # ID
    goal_s_disability_acceptance_txt: Optional[str] = Field(None, description="対応項目(障害受容): 詳細") # 無し
    goal_s_psychological_other_chk: Optional[bool] = Field(None, description="対応項目(心理面その他)の有無") # 無し
    goal_s_psychological_other_txt: Optional[str] = Field(None, description="対応項目(心理面その他): 詳細") # 無し
    goal_s_env_home_modification_chk: Optional[bool] = Field(None, description="対応項目(住宅改修)の有無") # 無し
    goal_s_env_home_modification_txt: Optional[str] = Field(None, description="対応項目(住宅改修): 詳細") # IR
    goal_s_env_assistive_device_chk: Optional[bool] = Field(None, description="対応項目(補装具)の有無") # 無し
    goal_s_env_assistive_device_txt: Optional[str] = Field(None, description="対応項目(補装具): 詳細") # IS
    goal_s_env_social_security_chk: Optional[bool] = Field(None, description="対応項目(社会保障)の有無") # 無し
    goal_s_env_social_security_physical_disability_cert_chk: Optional[bool] = Field(None, description="対応項目(社会保障): 身体障害者手帳") # 無し
    goal_s_env_social_security_disability_pension_chk: Optional[bool] = Field(None, description="対応項目(社会保障): 障害年金") # 無し
    goal_s_env_social_security_intractable_disease_cert_chk: Optional[bool] = Field(None, description="対応項目(社会保障): 難病医療費助成") # 無し
    goal_s_env_social_security_other_chk: Optional[bool] = Field(None, description="対応項目(社会保障): その他") # 無し
    goal_s_env_social_security_other_txt: Optional[str] = Field(None, description="対応項目(社会保障): その他詳細") # 無し
    goal_s_env_care_insurance_chk: Optional[bool] = Field(None, description="対応項目(介護保険)の有無") # 無し
    goal_s_env_care_insurance_details_txt: Optional[str] = Field(None, description="対応項目(介護保険): 詳細") # IW
    goal_s_env_care_insurance_outpatient_rehab_chk: Optional[bool] = Field(None, description="対応項目(介護保険): 通所リハ") # IX
    goal_s_env_care_insurance_home_rehab_chk: Optional[bool] = Field(None, description="対応項目(介護保険): 訪問リハ") # IY
    goal_s_env_care_insurance_day_care_chk: Optional[bool] = Field(None, description="対応項目(介護保険): 通所介護") # IZ
    goal_s_env_care_insurance_home_nursing_chk: Optional[bool] = Field(None, description="対応項目(介護保険): 訪問看護") # JA
    goal_s_env_care_insurance_home_care_chk: Optional[bool] = Field(None, description="対応項目(介護保険): 訪問介護") # 無し
    goal_s_env_care_insurance_health_facility_chk: Optional[bool] = Field(None, description="対応項目(介護保険): 老健") # 無し
    goal_s_env_care_insurance_nursing_home_chk: Optional[bool] = Field(None, description="対応項目(介護保険): 特養") # 無し
    goal_s_env_care_insurance_care_hospital_chk: Optional[bool] = Field(None, description="対応項目(介護保険): 介護医療院") # 無し
    goal_s_env_care_insurance_other_chk: Optional[bool] = Field(None, description="対応項目(介護保険): その他") # 無し
    goal_s_env_care_insurance_other_txt: Optional[str] = Field(None, description="対応項目(介護保険): その他詳細") # 無し
    goal_s_env_disability_welfare_chk: Optional[bool] = Field(None, description="対応項目(障害福祉)の有無") # 無し
    goal_s_env_disability_welfare_after_school_day_service_chk: Optional[bool] = Field(None, description="対応項目(障害福祉): 放課後等デイ") # 無し
    goal_s_env_disability_welfare_child_development_support_chk: Optional[bool] = Field(None, description="対応項目(障害福祉): 児童発達支援") # 無し
    goal_s_env_disability_welfare_life_care_chk: Optional[bool] = Field(None, description="対応項目(障害福祉): 生活介護") # 無し
    goal_s_env_disability_welfare_other_chk: Optional[bool] = Field(None, description="対応項目(障害福祉): その他") # 無し
    goal_s_env_other_chk: Optional[bool] = Field(None, description="対応項目(環境因子その他)の有無") # 無し
    goal_s_env_other_txt: Optional[str] = Field(None, description="対応項目(環境因子その他): 詳細") # 無し
    goal_s_3rd_party_main_caregiver_chk: Optional[bool] = Field(None, description="対応項目(主介護者)の有無") # 無し
    goal_s_3rd_party_main_caregiver_txt: Optional[str] = Field(None, description="対応項目(主介護者): 詳細") # 無し
    goal_s_3rd_party_family_structure_change_chk: Optional[bool] = Field(None, description="対応項目(家族構成変化)の有無") # 無し
    goal_s_3rd_party_family_structure_change_txt: Optional[str] = Field(None, description="対応項目(家族構成変化): 詳細") # 無し
    goal_s_3rd_party_household_role_change_chk: Optional[bool] = Field(None, description="対応項目(家庭内役割変化)の有無") # 無し
    goal_s_3rd_party_household_role_change_txt: Optional[str] = Field(None, description="対応項目(家庭内役割変化): 詳細") # 無し
    goal_s_3rd_party_family_activity_change_chk: Optional[bool] = Field(None, description="対応項目(家族活動変化)の有無") # 無し
    goal_s_3rd_party_family_activity_change_txt: Optional[str] = Field(None, description="対応項目(家族活動変化): 詳細") # 無し
